 ...
 * 本质：选择产品簇的实现。
 * 
 * 功能：为一系列相关对象或相互依赖的对象创建一个接口。这个接口内的方法不是任意堆砌的，而是一系列相互依赖的方法。从某种意义上看，抽象工厂其实是一个产品系列，或者是产品簇。
 * 
 * 使用工厂方法来实现抽象工厂。
 * 
 * 工厂方法是选择单个产品的实现，虽然一个类里面可以有多个工厂方法，但是这些方法之间一般没有联系的，即使看起来像有联系。
 ...
 * 2.如果一个系统要由多个产品系列中的一个来配置的时候。也就是可以动态地切换产品族的时候
 ...
 * 抽象工厂模式和单例模式
 * 这两个模式可以组合使用。
 * 在抽象工厂模式里面，具体的工厂实现，在整个应用中，通常一个产品系列只需要一个实现就可以了，因此可以把具体的工厂实现成为单例。
 */
